...
```bash
bash ~/intelligent_robot_practice/week03/run_pubsub.sh talker 202302200 강태욱
```

```bash
bash ~/intelligent_robot_practice/week03/run_pubsub.sh listener 202302200
```

```bash
bash ~/intelligent_robot_practice/week03/run_pubsub.sh graph
```

다른 워크스페이스를 사용하는 경우 `IRE_WS` 환경 변수에 해당 경로를 지정한다.
